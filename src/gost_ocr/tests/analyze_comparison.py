...
from ..config import YOLO_TEST_DIR, YOLO_TRAINED_MODEL, OUTPUT_DIR

YOLO_PRED_DIR = Path("runs/detect/predict-4/labels")
        ...
        if matching_opencv:
            try:
                with open(matching_opencv) as f:
                    data = json.load(f)
                    if "stamp_bbox" in data:
                        opencv_bbox = data["stamp_bbox"]
                        print(f"    OpenCV: {opencv_bbox} from {matching_opencv.name}")
